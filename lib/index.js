...
const log = debug('riot-lol-api:request');
const keepaliveAgent = new Agent();

class RiotRequest {
  static getPlatformFromRegion(region) {
  ...
  /**
   * Create a new instance for making request to the RiotAPI.
   ...
   */
  constructor(apiKey, cache, options) {
    if (!apiKey) {
    ...
    this.throttler = {};
    RiotRequest.PLATFORMS.forEach(p => (this.throttler[p.toLowerCase()] = {}));
  ...
  generateQueue(platform, method) {
    var self = this;
    var secondaryLog = debug('riot-lol-api:request:' + platform + ':' + method);
          ...
          .end(function(err, res) {
            if (
              ...
              if (bucketStartedAt === now) {
                ...
                defaultConcurrency = Math.min.apply(
                  Math,
                  limit.map(l => l[0])
                );
                timeBeforeReset =
            ...
            if (err && err.status === 429) {
              ...
              var retryAfter =
                (res.headers['retry-after'] ||
                  self.options.defaultRetryPeriod) * 1000;
   ...
   */
  request(platform, method, endpoint, cacheStrategy, done) {
    platform = platform.toLowerCase();
    method = method.toLowerCase();
  ...
  setThrottle(platform, method, throttle) {
    if (!throttle) {
      throttle = method;
      method = platform;
